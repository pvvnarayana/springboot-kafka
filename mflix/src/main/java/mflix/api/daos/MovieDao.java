...
public class MovieDao extends AbstractMFlixDao {
    ...
    private boolean validIdValue(String movieId) {
    	...
    	try {
    		Bson document = moviesCollection.find(Filters.eq("_id", new ObjectId(movieId))).first();
    		return document !=null ? true : false;
    	}catch(IllegalArgumentException e) {
    		System.out.println("exception in validIdValue");
    		e.printStackTrace();
    	}
    	return false;
    ...
    public Document getMovie(String movieId) {
        ...
        Bson lookupBson =   new Document("$lookup",new Document("from", "comments")
                .		append("let", 
                	    new Document("id", "$_id"))
    ...
    public List<Document> getMoviesByCountry(String... country) {
        Bson queryFilter = all("countries",Arrays.asList(country)); //new Document("countries",new Document("$all",Arrays.asList(country)));
        ...
        MongoCursor<Document> iterator = moviesCollection.find(queryFilter).projection(projection).iterator();
        List<Document> movies = new ArrayList<>();
        
        while(iterator.hasNext()) {
        	Document d = iterator.tryNext();
        	movies.add(d);
        }
        return movies;
    }
